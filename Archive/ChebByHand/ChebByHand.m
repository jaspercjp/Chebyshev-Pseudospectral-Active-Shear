...
[D1, y] = cheb(N, 1/2); D2=D1^2; D3=D1^3; D4=D1^4; 
...
B2 = eye(N+1) + (l/W)^2*(k^2*eye(N+1) - D2) + diag(1i*k*tBar*y);
C2 = tBar * eye(N+1);

A3 = (-lambda*tBar^3 / (1 + tBar^2)) * (k^2*eye(N+1) - (D2)) ...
    - lambda*tBar*(k^2*eye(N+1) + D2);
B3 = tBar * eye(N+1);
C3 = eye(N+1) + (l/W)^2*(k^2*eye(N+1) - D2) + diag(1i*k*tBar*y);
